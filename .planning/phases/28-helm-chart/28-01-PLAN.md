...
must_haves:
  truths:
    ...
    - "values.schema.json validates rds.managementIP and rds.secretName as required fields"
    - "_helpers.tpl provides name, fullname, chart, labels, selectorLabels, serviceAccountName helpers"
    - "helm lint passes on the chart directory with no errors"
  artifacts:
...
The existing manifests mount the secret at `/etc/rds-csi` and reference keys `rds-private-key` and `rds-host-key`. Keep these key names consistent.
  </action>
  <verify>
Run `ls -la deploy/helm/rds-csi-driver/` to confirm Chart.yaml, values.yaml, .helmignore exist. Verify Chart.yaml has apiVersion v2 and version 1.0.0.
  </verify>
  <done>Chart.yaml, values.yaml, and .helmignore exist with all configuration sections matching existing manifest values and main.go flag names.</done>
</task>
...
4. _helpers.tpl defines rds-csi.name, rds-csi.fullname, rds-csi.labels, rds-csi.selectorLabels, rds-csi.serviceAccountName
5. values.schema.json validates rds.managementIP and rds.secretName as required
</verification>
