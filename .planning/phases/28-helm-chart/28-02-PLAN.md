...
4. CSIDriver name is `rds.csi.srvlab.io` (not templated)
  ...
  <done>Four template files render correctly. RBAC preserves all existing rules. All namespace references use {{ .Release.Namespace }}. CSIDriver spec matches existing manifest exactly.</done>
</task>
...
5. **Container: rds-csi-driver** -- This is the main driver container:
   ...
   - Args (matching main.go flags EXACTLY):
     ...
     - `-rds-key-file=/etc/rds-csi/rds-private-key`
...
After creating both templates, run `helm template test-release deploy/helm/rds-csi-driver/ --namespace rds-csi` and verify the output matches the existing deploy/kubernetes/ manifests in structure (container names, volume mounts, security contexts). Differences should only be in names (templated) and namespace (templated).
  ...
  <verify>
Run `helm template test-release deploy/helm/rds-csi-driver/ --namespace rds-csi | grep -c "kind:"` to count generated resources. Run `helm lint deploy/helm/rds-csi-driver/` to verify no errors. Visually compare `helm template` output against existing deploy/kubernetes/ manifests to confirm all containers, volumes, and args are present.
  </verify>
  <done>Controller Deployment has 6 containers (driver + 5 sidecars) with all args from main.go templated. Node DaemonSet has 3 containers with privileged security context, hostNetwork, and bidirectional mount propagation preserved. helm template renders valid YAML matching existing manifest structure.</done>
</task>
...
6. All RBAC rules match deploy/kubernetes/rbac.yaml exactly
