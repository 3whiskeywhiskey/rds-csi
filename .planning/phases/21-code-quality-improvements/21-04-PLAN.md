...
must_haves:
  truths:
    ...
    - "v0.8.0 milestone marked as shipped"
  artifacts:
...
<objective>
Finalize Phase 21 and mark v0.8.0 milestone complete.
...
Output: Updated STATE.md and ROADMAP.md with Phase 21 complete and v0.8.0 shipped.
</objective>
...
Last activity: [DATE] - Completed Phase 21 with 4 plans, v0.8.0 shipped
```
...
```markdown
| v0.8.0 Code Quality and Logging Cleanup | 17-21 | 20/20 | Shipped [DATE] |
```
...
```
  ...
  <done>STATE.md updated with Phase 21 completion and v0.8.0 shipped status</done>
</task>

<task type="auto">
  <name>Task 2: Update ROADMAP.md to mark Phase 21 complete and v0.8.0 shipped</name>
...
```markdown
- ✅ **v0.8.0 Code Quality and Logging Cleanup** - Phases 17-21 (shipped [DATE])
```

2. Update Phase 21 section - CRITICAL: Correct success criteria to accurately reflect what Phase 21 delivers:
```markdown
### Phase 21: Code Quality Improvements
...
This corrects the requirements/success criteria mismatch by:
- Removing QUAL-01 and QUAL-03 from Phase 21 requirements (they're handled elsewhere)
- Listing only the 3 items Phase 21 actually delivers in Success Criteria
- Adding explicit "Prior/Deferred Items" section to document where QUAL-01/QUAL-03 are handled

3. Update Progress table:
```markdown
| 21. Code Quality Improvements | v0.8.0 | 4/4 | Complete | [DATE] |
```

4. Update timestamp at bottom:
```markdown
_Last updated: [DATE] (Phase 21 complete - v0.8.0 shipped)_
```
  </action>
  <verify>grep "21-01-PLAN.md.*\[x\]" .planning/ROADMAP.md shows plan marked complete</verify>
  <done>ROADMAP.md updated with Phase 21 complete, success criteria corrected to reflect actual deliverables, and v0.8.0 milestone shipped</done>
</task>
...
5. Push to dev branch:
  ...
  <done>Milestone committed and pushed, v0.8.0 complete</done>
</task>
...
3. `grep "v0.8.0.*shipped" .planning/ROADMAP.md` shows milestone complete
4. `git log -1` shows milestone commit
</verification>

<success_criteria>
1. STATE.md shows Phase 21 of 21 complete
2. ROADMAP.md v0.8.0 milestone marked shipped
3. All tests pass
