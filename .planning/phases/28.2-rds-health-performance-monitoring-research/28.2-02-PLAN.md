...
```go
    ...
    if config.EnableController && config.Metrics != nil && driver.rdsClient != nil {
        ...
        // SNMP target configuration (use DPU VLAN IP that's reachable from dev machine)
        // In production, this might be the storage VLAN IP if Prometheus can reach it
        snmpHost := "10.42.67.1"  // TODO: make configurable via helm values
...
- **SNMP on DPU VLAN:** Uses 10.42.67.1 (DPU VLAN) for SNMP which is reachable from dev machine.
  Storage VLAN (10.42.68.1) is unreachable but could be used in production if Prometheus can reach it.
...
Currently hardcoded to monitor `storage-pool` slot via SSH at 10.42.68.1 (via mgmt VRF 10.42.241.3)
and hardware via SNMP at 10.42.67.1. Future Helm chart values:
...
monitoring:
  ...
  snmp:
    host: "10.42.67.1"        # SNMP target (DPU VLAN or storage VLAN)
...
6. **Network topology dependency:** SNMP uses DPU VLAN IP (10.42.67.1) which is reachable from dev machine.
   Production deployment may need different IP if Prometheus runs in different network segment.
