...
func (m *Metrics) SetAttachmentManager(countFunc func() int) {
    ...
    nvmeConnectionsActive := prometheus.NewGaugeFunc(
        prometheus.GaugeOpts{
            ...
            Help:      "Number of currently active NVMe/TCP connections (from attachment manager state)",
        },
        func() float64 {
...
IMPORTANT: Add SetAttachmentManager to this test to ensure nvme_connections_active is included in namespace validation.
  ...
  <verify>
Run `make test` and verify all tests pass. Run `go test -v ./pkg/observability/... -run "NVMeConnect|NVMeDisconnect|NVMeConnectionsActive|MetricsNamespace"` to specifically check the updated/new tests.
  </verify>
  <done>
