...
<task type="auto">
  ...
  <action>
Modify manager.go to support RWX dual-attachment. IMPORTANT: Do NOT change the existing TrackAttachment signature - this would break all existing callers.

1. Keep existing TrackAttachment signature unchanged - it will continue to work for RWO attachments:
...
2. Add new TrackAttachmentWithMode for RWX-aware tracking:
...
3. Add AddSecondaryAttachment for RWX migration:
...
5. Add RemoveNodeAttachment for partial detach:
...
```
  ...
  <verify>
Run `go build ./pkg/attachment/...` to verify no syntax errors.
Run `make test` to ensure existing tests pass (existing TrackAttachment callers unchanged).
  </verify>
  <done>
- TrackAttachment signature UNCHANGED for backward compatibility
- TrackAttachmentWithMode added for access-mode-aware tracking
...
- GetNodeCount, IsAttachedToNode, GetAccessMode helpers added
  </done>
</task>
...
<task type="auto">
  ...
  <action>
Modify ControllerPublishVolume to handle RWX dual-attachment. Use inline access mode check since ControllerPublishVolume uses singular VolumeCapability (not the plural slice used in CreateVolume).

1. Update ControllerPublishVolume to handle RWX differently (starting around line 446):
...
func (cs *ControllerServer) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (*csi.ControllerPublishVolumeResponse, error) {
    ...
    // ... existing VMI lock acquisition (keep as-is) ...
    ...
    if exists {
        ...
        // RWO: Check grace period first (existing behavior - keep as-is)
    ...
    // ... rest of existing code (metrics, events, return) ...
}
```

2. Update ControllerUnpublishVolume to use RemoveNodeAttachment (starting around line 594):
...
NOTE: The access mode is determined inline from `req.GetVolumeCapability().GetAccessMode().GetMode()` - NOT from a helper function with mismatched types.
  </action>
  <verify>
...
Run `make test` to ensure existing tests pass.
  ...
  <done>
- ControllerPublishVolume determines RWX access mode inline from singular VolumeCapability
...
- ControllerUnpublishVolume uses RemoveNodeAttachment for partial detach support
...
- TrackAttachment signature unchanged (backward compatible)
