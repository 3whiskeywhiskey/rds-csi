...
must_haves:
  ...
  key_links:
    ...
    - from: "CreateVolume VolumeContext"
      to: "ControllerPublishVolume req.GetVolumeContext()"
      via: "CSI VolumeContext parameter flow"
      pattern: "migrationTimeoutSeconds"
---
...
**VolumeContext flow verification:** The CSI spec guarantees VolumeContext from CreateVolumeResponse is stored in the PV and passed to ControllerPublishVolume via req.GetVolumeContext(). This is how existing parameters like "nvmeAddress", "nqn", etc. already flow through. Task 1 accesses this via `ParseMigrationTimeout(req.GetVolumeContext())`.
  </action>
  <verify>
1. grep -n "migrationTimeoutSeconds" pkg/driver/controller.go shows it in CreateVolume response
2. grep -A3 "VolumeContext" pkg/driver/controller.go shows BOTH CreateVolume setting migrationTimeoutSeconds AND existing ControllerPublishVolume accessing req.GetVolumeContext() (confirms the parameter actually flows through)
  </verify>
...
6. Verify VolumeContext flow: grep -A3 "VolumeContext" pkg/driver/controller.go shows both CreateVolume setting it and ControllerPublishVolume reading it
</verification>
...
- VolumeContext flow verified (CreateVolume sets, ControllerPublishVolume reads)
