...
must_haves:
  truths:
    ...
    - "Snapshot create/delete/list operations work end-to-end through CSI gRPC -> driver -> mock SSH -> mock RDS"
    - "Hardware validation TC-08 documents the copy-from snapshot approach and is ready for manual execution against real RDS"
    - "TC-08 SSH verification commands use /disk print detail (not /disk/btrfs/subvolume/print)"
...
Output: Passing CSI sanity tests (including snapshots); updated TC-08 hardware validation test case ready for manual execution.
</objective>
...
<task type="auto">
  ...
  <files>test/sanity/sanity_test.go, test/mock/rds_server.go, pkg/driver/controller.go</files>
  <action>
...
**Files that may need fixes:** This task primarily targets `test/sanity/sanity_test.go` but may also require fixes in `test/mock/rds_server.go` (if Plan 01's mock doesn't handle edge cases the sanity suite exercises) or `pkg/driver/controller.go` (if the CSI RPCs have bugs revealed by sanity testing).
...
2. **Snapshot not found after create**: Mock server's `/disk print detail where slot=snap-*` may not return output in the format parseSnapshotInfo() expects. Check that the mock's formatSnapshotDetail output matches what parseSnapshotInfo parses (slot="...", file-path="...", file-size=NNN).
...
5. **DeleteSnapshot fails**: Ensure handleDiskRemove checks both s.volumes and s.snapshots maps (should be handled by Plan 01, but verify).
...
**Hardware execution note:** Add a section at the end of TC-08 titled "Execution" with: "This test case is designed for manual execution against real RDS hardware. After automated sanity tests pass (this phase), execute TC-08 steps manually against the RDS at 10.42.68.1 and record results in the Results Template. Document execution outcome in the phase SUMMARY.md."
...
Grep for "read-only=yes" in docs/HARDWARE_VALIDATION.md — should return 0 matches (in TC-08 context).
  ...
  <done>
TC-08 documents the copy-from snapshot approach: SSH verification uses `/disk print detail`, expected output shows file-backed disk without NVMe export, troubleshooting references copy-from semantics. Includes execution note for manual hardware validation. No Btrfs subvolume references remain.
  </done>
</task>
...
7. TC-08 includes execution note for manual hardware validation
</verification>
...
- TC-08 is documented as ready for manual hardware execution (post-phase activity)
</success_criteria>
